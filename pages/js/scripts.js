...
function realWordCount() {
  if (box.value.trim().split(/\s+/).length == 1 && box.value.length > 0) {
    charDisplay.innerHTML = "word";
  } else {
    charDisplay.innerHTML = "words";
  }
  if (box.value.length < 1) {
    return 0;
  } else {
    return box.value.trim().split(/\s+/).length;
  }
}
...
function typeWriter() {
  if (i < prompt.length) {
    box.placeholder += prompt.charAt(i);
    i++;
